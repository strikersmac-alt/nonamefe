...
export default function WaitingRoom() {
  ...
  useEffect(() => {
    ...
    socketInstance.on('contestStarted', (data) => {
      ...
      setError(null);
      navigate(`/contest/${contestId}/play`, { 
        state: { contestMeta: { ...contestMeta, isLive: true, startTime: data.startTime } } 
      });
    });
  ...
  const getModeDisplay = (mode: string) => {
    switch (mode) {
      case 'duel':
        return { text: 'Duel (2 Players)', color: 'orange', limit: 2 };
      case 'multiplayer':
  ...
  const modeInfo = contestMeta ? getModeDisplay(contestMeta.mode) : { text: '', color: 'gray', limit: null };
  const canStart = modeInfo.limit ? participants.length >= modeInfo.limit : participants.length > 0;
  ...
  return (
    <Box style={{
      ...
      <Container size="4" px={{ initial: '4', sm: '5', md: '6' }} pt={{ initial: '4', md: '5' }} pb="6" style={{ flex: 1, position: 'relative', zIndex: 1 }}>
        
        {/* Header Section */}
        <Flex direction="column" align="center" gap="4" mb="6">
          <Badge size="3" color={connected ? 'green' : 'red'} variant="soft" style={{ backdropFilter: 'blur(10px)' }}>
          ...
          {error && (
            <Card style={{
              ...
              <Flex direction="column" align="center" gap="3">
                <Text size="3" weight="bold" style={{ color: '#ef4444', textAlign: 'center' }}>
                  {error}
                </Text>
                <Button
                  size="2"
          ...
          <Flex direction="column" gap="4">
            <Flex justify="between" align="center">
              <Heading size="5" style={{ color: '#e2e8f0', fontWeight: 700 }}>
                <PersonIcon style={{ display: 'inline', marginRight: '0.5rem' }} />
                Participants ({participants.length}{modeInfo.limit ? `/${modeInfo.limit}` : ''})
              </Heading>
            ...
            {participants.length === 0 ? (
              <Card style={{
                ...
                <Text size="2" style={{ color: 'rgba(226, 232, 240, 0.8)', marginTop: '0.5rem' }}>
              ...
              <Grid columns={{ initial: '1', sm: '2', md: '3' }} gap="3">
                {participants.map((participant, index) => (
                  <Card key={participant.userId} className="feature-card" style={{
                    ...
                    <Flex align="center" gap="3">
                      ...
                      <Flex direction="column" style={{ flex: 1 }}>
                        <Text size="3" weight="medium" style={{ color: '#e2e8f0', fontWeight: 600 }}>
                          {participant.name}
                        </Text>
                        {index === 0 && (
                          <Badge size="1" color="violet" variant="soft" style={{ width: 'fit-content' }}>
                            Admin
                          </Badge>
                        )}
                      </Flex>
        ...
        {!error && isAdmin && (
          <Flex justify="center" mt="6">
            <Button
              ...
              style={{
                background: canStart ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'rgba(100, 100, 100, 0.3)',
                cursor: canStart && connected ? 'pointer' : 'not-allowed',
                fontWeight: 700,
                fontSize: '1.1rem',
                padding: '1.5rem 3rem',
                boxShadow: canStart ? '0 8px 24px rgba(16, 185, 129, 0.4)' : 'none',
              }}
            >
              <RocketIcon width={20} height={20} />
              {canStart ? 'Start Contest' : `Waiting for ${modeInfo.limit ? modeInfo.limit - participants.length : 1} more player(s)`}
            </Button>
          </Flex>
        )}
